...
Providers implement `complete(request, signal?)`. Provider-specific clients, credentials, and transmission policy remain outside this package.

`runAIOperation()` validates requests and responses. Failures are reported as `AdocForgeAIError` with one of these codes:
